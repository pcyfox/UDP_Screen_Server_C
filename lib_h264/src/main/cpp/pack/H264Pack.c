...
Result GetSPS_PPS_RTP_STAP_Pkt(const unsigned long ts, unsigned long clock) {
    if (stapA != NULL) {
        if (headerLen == RTP_LITE_HEADER_LEN) {
            return AddRTPLiteHeader(stapA->pkt, stapA->len);
        } else {
...
int PackRTP(unsigned char *h264Pkt,
    ...
    if (length <= maxPktLen - headerLen) {
        if (isLiteMod) {
            callback(AddRTPLiteHeader(h264Pkt, length));
        } else {
        ...
        while (currentIndex < length) {
            ...
            Result result = NULL;
            if (isLiteMod) {
                result = AddRTPLiteHeader(buf, bufSize);
            } else {
                result = AddRTPHeader(ts, clock, buf, bufSize);
            }
            free(buf);
            buf = NULL;
